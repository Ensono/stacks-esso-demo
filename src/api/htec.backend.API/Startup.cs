...
using htec.backend.API.Authentication;
using htec.backend.API.Authorization;
using htec.backend.API.Models.Requests;

namespace htec.backend.API
{
    public class Startup
    {
            ...
            IOptions<JwtBearerAuthenticationConfiguration> jwtBearerAuthenticationOptions)
        {
            ...
            .UseSwaggerUI(c =>
            {
                ...
                c.SwaggerEndpoint("all/swagger.json", "DOMAIN (all)");
                c.SwaggerEndpoint("v1/swagger.json", "DOMAIN (version 1)");
                c.SwaggerEndpoint("v2/swagger.json", "DOMAIN (version 2)");
            ...
            services
                ...
                .AddSwaggerGen(c =>
                {
                    c.SwaggerDoc("all", new OpenApiInfo
                    {
                        Version = "all",
                        Title = "DOMAIN API",
                        Description = "APIs used to interact and manage domains for a restaurant",
                    ...
                    c.IncludeXmlComments($"{AppContext.BaseDirectory}{Path.DirectorySeparatorChar}{typeof(CreateDOMAINRequest).Assembly.GetName().Name}.xml");

                    // Set default tags, shows on top, non defined tags appears at bottom
                    c.DocumentFilter<SwaggerDocumentTagger>(new OpenApiTag[] {
                        new OpenApiTag { Name = "DOMAIN" },
